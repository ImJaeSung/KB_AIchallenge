...
def getAiAnswer(df, question):
    ...
    example_mydata = '나이는 28세, 직업은 직장인, 결혼여부은 미혼, 신용은 높음, 예금여부는 예, 대출여부는 예 '
    # my_textual_data = "나이은 43세, 직업은 육체노동직, 결혼여부은 미혼, 신용은 높음, 예금여부은 예, 대출여부은 예" # for debugging
    definition_first = postprocessing(definition)

    full_query = example_mydata + " " + question + " " + definition_first
    #%%
